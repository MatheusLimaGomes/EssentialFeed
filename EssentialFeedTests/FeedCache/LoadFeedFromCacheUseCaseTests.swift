...
//  EssentialFeedTests
//
//  Created by Matheus Gomes on 25/02/24.
//
...
final class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversNoImageOnCacheExpiration() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let expirationCacheTimestamp = fixedCurrentDate.minusFeedCacheMaxAge()
        let (sut, store) = makeSUT(currentDate: { fixedCurrentDate })
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWith expectedResult: FeedLoader.Result,
