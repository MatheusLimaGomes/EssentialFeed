...
final class FeedViewControllerTests: XCTestCase {
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 2, "Expected another loading request once user initiated another load.")
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no imageURL requests views become visible")
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected second image URL request once first second view also become visible.")
    }
    func test_feedImageView_cancelImageURLWhenNotVisibleAnymore() {
        let image0 = makeImage(url: URL(string: "https://image-0.com")!)
        let image1 = makeImage(url: URL(string: "https://image-1.com")!)
        let (sut, loader) = makeSUT()
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected cancelled one image URL request once image is not visible anymore.")
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view0?.renderedImage, image0, "Expected image for first view once first image loading completes succesfully")
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image loading completes succcesfully.")
        
        let image1 = UIImage.make(withColor: .red).pngData()!
        loader.completeImageLoading(with: image1, at: 1)
        XCTAssertEqual(view1?.renderedImage, image1, "Expected image for second view once second image loading completes succesfully.")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadedError() {
        ...
        XCTAssertEqual(view0.isShowingRetryAction, false, "Expected no retry action for the first view once the loading image completes successfuly.")
        XCTAssertEqual(view1.isShowingRetryAction, false, "Expected no retry action for the second view while loading second image.")
        
        loader.completeImageLoadingWirhError(at: 1)
        XCTAssertEqual(view0.isShowingRetryAction, false, "Expected no retry action for the first view while loading first image.")
        XCTAssertEqual(view1.isShowingRetryAction, true, "Expected a retry action for the second view once  second image loading completes with error.")
    }
    ...
    func test_feedImageViewRetryButton_retriesImageLoad() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected no retry action for the first view while loading first image.")
    ...
    private class LoaderSpy: FeedLoader, FeedImageDataLoader {
        ...
        func completeImageLoadingWirhError(at index: Int = 0) {
            let error = NSError(domain: "Error loading Image", code: 0)
